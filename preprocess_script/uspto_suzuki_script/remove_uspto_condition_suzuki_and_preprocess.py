...
if __name__ == "__main__":
    ...
    uspto_condition_dataset_with_rxn_category = pd.read_csv(
        os.path.join(
            source_data_path,
    ...
    uspto_condition_dataset_rm_suzuki_data = (
        uspto_condition_dataset_with_rxn_category.loc[
            uspto_condition_dataset_with_rxn_category["rxn_category"]
            != float(name2rxn_class["Suzuki coupling"])
        ].reset_index(drop=True)
    )

    uspto_condition_dataset_rm_suzuki_data_save_path = os.path.join(
        source_data_path, "USPTO_condition_final_rm_suzuki"
    )
    ...
    uspto_condition_dataset_rm_suzuki_data.to_csv(
        os.path.join(
            uspto_condition_dataset_rm_suzuki_data_save_path,
            "USPTO_condition_pred_category.csv",
        ),
        index=False,
    )

    uspto_condition_dataset_rm_suzuki_data_rm_category = (
        uspto_condition_dataset_rm_suzuki_data.drop(columns=["rxn_category"])
    )

    uspto_condition_dataset_rm_suzuki_data_rm_category.to_csv(
        os.path.join(
            uspto_condition_dataset_rm_suzuki_data_save_path, "USPTO_condition.csv"
        ),
        index=False,
    )
    ...
    np.savez_compressed(
        os.path.join(
            final_condition_data_path, "{}_rxn_fps".format(database_fname.split(".")[0])
        ),
        fps=rxn_fps,
    )
