...
def main(parser_args, debug=False):
    pandarallel.initialize(nb_workers=parser_args.num_workers, progress_bar=True)
    ...
    test_df = pd.DataFrame(
        {
            ...
            "labels": (
                [[0] * 7] * len(input_rxn_smiles)
                if not model_args["use_temperature"]
                else [[0] * 8] * len(input_rxn_smiles)
            ),
        }
    )
    print("Canonicalize reaction smiles and remove invalid reaction...")
    test_df["text"] = test_df.text.parallel_apply(lambda x: caonicalize_rxn_smiles(x))
    ...
    output_df = get_output_results(
        test_df.text.tolist(), pred_conditions, pred_temperatures
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inference_batch_size", default=8, help="Batch size", type=int
    )
