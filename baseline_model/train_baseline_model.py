...
def load_dataset(data_root, database_name, use_temperature=False):
    ...
    prod_fps = np.load(os.path.join(data_root, f"{database_name}_prod_fps.npz"))["fps"]
    rxn_fps = np.load(os.path.join(data_root, f"{database_name}_rxn_fps.npz"))["fps"]
    ...
    for dataset in list(set(database_df["dataset"].tolist())):
        dataset_prod_fps = prod_fps[
            database_df.loc[database_df["dataset"] == dataset].index
        ]
        ...
        if use_temperature:
            dataset_temperature = database_df.loc[database_df["dataset"] == dataset][
                "temperature"
            ].values
            temperature_dict[dataset] = dataset_temperature
            print(
                "{} temperature array shape: {}".format(
...
class ConditionDataset(Dataset):
    def __init__(self, fps_dict, label_dict, dataset, use_temperature):
        ...
        self.label_names = ["catalyst1", "solvent1", "solvent2", "reagent1", "reagent2"]
        ...
        if use_temperature:
            self.temperature = torch.tensor(label_dict["temperature"][dataset]).float()
...
def train_one_epoch(
    ...
    for data in tqdm(train_loader):
        ...
        loss_s2 = loss_fn_list[0](model.s2_func(fp_emb, input_c1, input_s1), s2)
        batch_loss_list += [loss_s2]
        input_s2 = get_one_hot_input(s2, dim=len(condition2label["solvent2"]))
        loss_r1 = loss_fn_list[0](
            model.r1_func(fp_emb, input_c1, input_s1, input_s2), r1
        )
        batch_loss_list += [loss_r1]
        input_r1 = get_one_hot_input(r1, dim=len(condition2label["reagent1"]))
        loss_r2 = loss_fn_list[0](
            model.r2_func(fp_emb, input_c1, input_s1, input_s2, input_r1), r2
        )
        batch_loss_list += [loss_r2]

        if use_temperature:
            input_r2 = get_one_hot_input(r2, dim=len(condition2label["reagent2"]))
            loss_t = loss_fn_list[1](
                model.T_func(
                    fp_emb, input_c1, input_s1, input_s2, input_r1, input_r2
                ).squeeze(),
                t,
            )
...
def validation(
    ...
    if use_temperature:
        print(
            "Temperature MSE: {}".format(temp_mse / len(data_loader.dataset))
        )  # mse reduce 是默认的mean
        ...
        with torch.no_grad():
            ...
            for idx in range(sorted_one_batch_preds.size(0)):
                topk_hit_df = get_accuracy_for_one(
                    sorted_one_batch_preds[idx],
                    one_batch_ground_truth[idx],
                    topk_get=topk_get,
...
def load_model(model=None, model_dir=""):
    state = torch.load(
        os.path.join(model_dir, "model.pth"), map_location=torch.device("cpu")
    )
    if model:
...
if __name__ == "__main__":
    ...
    # config_path = './baseline_config/baseline_config_uspto_suzuki.yaml'
    config_path = "./baseline_config/baseline_config_uspto_suzuki_transfer_from_uspto_condition.yaml"
    ...
    if debug:
        config["database_path"] = (
            "../dataset/source_dataset/USPTO_condition_final_debug"
        )
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.7, patience=2, min_lr=0.000001
    )
    ...
    if not test_model:
        ...
        print("Testing model...")
        model = load_model(
            model, os.path.join(config["model_path"], config["model_name"])
        )
        model = model.to(device)
        caculate_accuracy(
            model,
